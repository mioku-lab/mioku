...
export function registerInstallCommands(ctx: MiokiContext): () => void {
  const dispose = ctx.handle("message", async (event: any) => {
    ...
    if (!text.startsWith(installHead) && !text.startsWith(uninstallHead)) return;
    ...
    if (text.startsWith(installHead)) {
      ...
      if (!parsed) {
        await replyText(event, `用法：${prefix}install plugin <名称> 或 ${prefix}install service <名称>`);
        return;
      }
      await replyText(event, `正在从 npm 安装${typeLabel(parsed.type)} ${parsed.name}...`);
      try {
        ...
        const lines = [
          ...
          result.enabled ? "已在 mioki.plugins 中启用" : "",
          "重启后生效。",
        ].filter(Boolean);
    ...
    if (!parsed) {
      await replyText(event, `用法：${prefix}uninstall plugin <名称> 或 ${prefix}uninstall service <名称>`);
      return;
    }
    await replyText(event, `正在卸载${typeLabel(parsed.type)} ${parsed.name}...`);
    try {
      ...
      const lines = [
        ...
        result.removedFromConfig ? "已从 mioki.plugins 中移除" : "",
        "重启后生效。",
      ].filter(Boolean);
      await replyText(event, lines.join("\n"));
    } catch (error) {
