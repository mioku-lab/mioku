  ...
  const bot: OneBotBase = {
    ...
    async sendMessage(
      ...
      if (target.type === "group" && target.group_id) {
        const sent = await api("send_group_msg", {
      ...
      if (target.type === "private" && target.user_id) {
        const sent = await api("send_private_msg", {
    ...
    async getFriendInfo(userId: string) {
      try {
        const result = await api<Record<string, unknown>>("get_stranger_info", {
          user_id: userId,
        });
        return {
          ...result,
    ...
    async leaveGroup(groupId: string, isDismiss?: boolean) {
      await api("set_group_leave", {
        group_id: groupId,
        is_dismiss: isDismiss ?? false,
      });
    },
    ...
    async getCookie(domain: string) {
      const { cookies, bkn } = await api<{ cookies: string; bkn: string }>(
        "get_cookies",
        { domain },
      );
    ...
    async getVersionInfo() {
      return (await api("get_version_info")) as {
        app_name: string;
        app_version: string;
        protocol_version: string;
      };
    ...
    async getLoginInfo() {
      return (await api("get_login_info")) as {
        user_id: number;
        nickname: string;
      };
    },
    ...
    async getMessage(messageId: string) {
      try {
        const result = await api<Record<string, unknown>>("get_msg", {
        ...
        return {
          ...
          message: buildSegments(
            ...
            typeof result.raw_message === "string"
              ? result.raw_message
              : undefined,
          ),
        } as MessageGetResult;
    ...
    async getForwardMessage(messageId: string) {
      const result = await api<{ messages?: Record<string, unknown>[] }>(
        "get_forward_msg",
        { id: messageId },
      );
    ...
    async getGroupInfo(groupId: string) {
      try {
        const result = await api<Record<string, unknown>>("get_group_info", {
          group_id: groupId,
        });
    ...
    async getHistory(
      ...
      let raw: unknown;
      const common = { count: limit, ...extra };
      if (target.type === "group" && target.group_id) {
        raw = await api("get_group_msg_history", {
          group_id: target.group_id,
          message_seq: before ?? 0,
          ...common,
        });
