import { isMessage, MessageSegmentImpl } from "mioku";
...
const normalize = (item: string | MessageSegment): SendNormalizedSegment => {
  ...
  if (type === "at") {
    // Core segments carry the target as `target`; OneBot v11 expects `qq`.
    // NapCat drops at segments with a missing qq, so normalize both shapes here.
    const qq = segData.qq ?? segData.target;
    return { type: "at", data: { qq: qq == null ? "" : String(qq) } };
  }
  if (type === "reply") {
    // Core segments carry the message id as `message_id`; OneBot v11 expects `id`
    // (NapCat reads id/seq and silently drops the reply otherwise).
    const id = segData.id ?? segData.message_id;
    const replyData: Record<string, unknown> = {};
    if (id != null && String(id) !== "") replyData.id = String(id);
    if (segData.seq != null) replyData.seq = segData.seq;
    return { type: "reply", data: replyData };
  }
...
export const sentFromOneBot = (
  data: { message_id?: number | string } | undefined,
): SentMessage => {
  if (!data) return {};
...
export const attachmentFromImageData = (
  data: Record<string, unknown>,
  ...
  return {
    url,
    file,
    name: typeof data.file_unique === "string" ? data.file_unique : undefined,
  };
};
